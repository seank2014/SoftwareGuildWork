        ...
        $.ajax({
            ...
            success: function(data, status) {
               ...
               // Clear the form and reload the table
                $('#add-first-name').val('');//setting values to empty string so it will empty out
                $('#add-last-name').val('');
                $('#add-company').val('');
                $('#add-phone').val('');
                $('#add-email').val('');
            ...
            error: function() { //error doesnt take any parameters
                $('#errorMessages')
                   .append($('<li>')
...
function loadContacts() {
    ...
    $.ajax ({
        ...
        success: function (data, status) {
            $.each(data, function (index, contact) {
                ...
                var row = '<tr>';
                    row += '<td>' + name + '</td>';
                    row += '<td>' + company + '</td>';
                    row += '<td><a onclick="showEditForm(' + id + ')">Edit</a></td>'; //rendering edit link
        ...
        error: function() {
            $('#errorMessages')
                .append($('<li>')
                .attr({class: 'list-group-item list-group-item-danger'}) //class attribute
                .text('Error calling web service.  Please try again later.'));
        }
    });
}
